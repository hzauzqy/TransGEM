...
def out_decode(model, data, device, args):  
    ...
    x = model.TransGEM.gene_embedding(x)
    x = model.TransGEM.position(x)
    softmax=nn.Softmax(dim=1)
    for _ in range(args.max_len):
        all_seq=[]
        for i in y_pred:
            if _==0:
                y = model.TransGEM.tgt_embedding(i)
                y = model.TransGEM.position(y)
                out = model.TransGEM.decoder(y, x, None, None)
                out = model.fine(out)
                prob = softmax(model.TransGEM.generator(out[:, -1])).detach()
                prob=prob.cpu()
                ...
                if args.end_idx in i[1].cpu().numpy().tolist():
                    ...
                    y = model.TransGEM.tgt_embedding(i[1].unsqueeze(0).to(device))
                    y = model.TransGEM.position(y)
                    out = model.TransGEM.decoder(y, x, None, None)
                    out = model.fine(out)
                    prob = softmax(model.TransGEM.generator(out[:, -1])).detach()
...
ckpt = torch.load("ckpt/best_ckpt_{}".format(args.dataset), map_location=torch.device('cpu'))
